<template>
  <section>
    ...
    <div class="limit_coupon_label" v-if="http.res.coupon.useless.length > 0">不可用红包 ({{http.res.coupon.useless.length}}张)</div>

    <div class="box" v-for="coupon in http.res.coupon.useless">
    ...
    <div class="limit_coupon_label" v-if="http.res.coupon.invalid.length > 0">已过期红包 ({{http.res.coupon.invalid.length}}张)</div>

    <div class="box" v-for="coupon in http.res.coupon.invalid">
      <div class="coupon coupon_white box_radius_header">
        <div class="coupon_title">
          <div class="coupon_title_label coupon_disable">优惠券</div>
          <div class="coupon_title_charge">
  ...
  export default {
    ...
    methods: {
        ...
        httpCouponApi.getAll(this.$route.params.shortId, 0, 99).then(res => {
          ...
          for (let index in res.elements) {
            ...
            if (Boolean(coupon.orderId)) {
              this.http.res.coupon.invalid.push(coupon)
            } else {
              if (new Date().getTime() > coupon.expiredAt) {
                this.http.res.coupon.invalid.push(coupon)
              } else {
                if (this.$route.query.price > coupon.chargePrice) {
                  this.http.res.coupon.useless.push(coupon)
                } else {
      ...
      btnChooseCoupon(coupon) {
        this.$router.push({
          path: `/m/${this.$route.params.shortId}/order/${this.$route.query.orderOneId}`,
          query: {
      ...
      btnChooseNone() {
        this.$router.push(`/m/${this.$route.params.shortId}/order/${this.$route.query.orderOneId}`)
      }
    }
  }
</script>
